...
def account_balances_as_of(cur, fund_id: str, as_of: date, investor_id: str | None = None) -> dict:
    """Signed balance per account, replaying only posted journal lines on or before `as_of`.
    ...
    no special-casing here: a reversal is just another posted entry, so it nets
    against the original automatically once both are included in the replay.
    """
    query = """
